...
export default function TaskCard({ task, canEdit }: TaskCrdProps) {
    ...
    return (
        <>
            <li className="p-5 bg-white border border-slate-300 rounded-lg flex justify-between gap-3 dark:bg-slate-600 dark:border dark:border-gray-600">
                <div

                    className="min-w-0 flex-col gap-y-4">
                    <button
                        type="button"
                        className="text-xl font-bold text-slate-600 text-left dark:text-white"
                        onClick={() => navigate(location.pathname + `?viewTask=${task._id}`)}
                    >{task.name}</button>
                    <p className="text-slate-500 font-medium pt-1 dark:text-gray-400">{task.description}</p>
                </div>
